...
- **Video Looping**: Scene group videos loop seamlessly in final renders using actual duration
...
### Extension Ideas
- ✅ Support for video clips instead of static images (Grok video generation + import)
- ✅ Lyric overlay on video (Karaoke-style lyrics during playback)
- ✅ Web UI for configuration (React + Shadcn/ui)
...
## Remotion Web App Features

### Outro/Credits Sequence

**Location**: `src/remotion/Outro.tsx`, `src/remotion/VideoComposition.tsx`

Automatically generates a professional credits sequence at the end of the video:

- **Video Grid**: Dynamic grid layout that scales to accommodate any number of videos
- **Ripple Animation**: Videos appear with staggered timing from top-left to bottom-right
- **Grid Calculation**: `getGridDimensions()` calculates optimal cols/rows based on video count
- **Looping**: Uses Remotion's `<Loop>` component for continuous playback
- **AI Attribution**: Displays "Videos by Grok • Music by Suno AI • Lyrics by Claude, Grok & ChatGPT"
- **Branding**: Shows app name and GitHub URL with fade transitions
- **Configuration**: `OutroConfig` type in `src/types/index.ts`, toggleable in Display Configuration

### Video Looping in Scene Groups

**Location**: `src/remotion/SceneGroup.tsx`, `src/lib/media-utils.ts`

Videos now loop correctly in final renders using actual video duration:

- **Duration Detection**: `detectVideoDuration()` in `media-utils.ts` reads duration from video metadata
- **MediaVersion Type**: `duration?: number` field stores detected duration
- **Loop Calculation**: `videoLoopDuration = Math.ceil(videoDurationSeconds * fps)`
- **Conditional Looping**: Only loops when `sceneGroup.duration > videoDurationSeconds`
- **Detection Timing**: Duration detected when videos are added via `addMediaVersion()` or `handleImportVideo()`

### AI Enhanced Prompts

**Location**: `src/lib/image-api.ts` (`enhancePromptWithAI` function)

Precision visual translator framework for better image prompts:

- **Framework**: Four-question analysis (literal meaning, emotion, objects/metaphors, symbolic image)
- **Banned Terms**: vibrant, cinematic, gritty, raw, rebellious, neon, alley, graffiti, fire escape, haze, smoke, pulsing, crackling, high-contrast drama, urban skyline, motorcycle, lightning
- **Rules**: Prefer symbolic/surreal over realistic; color palette serves emotion only
- **Progressive UI**: Updates after each batch using accumulated `allGroupUpdates` object
- **Theme Context**: Suno style, genre, and mood appended for consistency across scenes
